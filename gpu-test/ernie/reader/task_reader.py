...
#        text_a = tokenization.convert_to_unicode(example.text_a)
        text_a = tokenization.convert_to_unicode(example.sentence)
        ...
        if "aspect" in example._fields:
            text_b = tokenization.convert_to_unicode(example.aspect)
    ...
    def _prepare_batch_data(self, examples, batch_size, phase=None):
        ...
        for index, example in enumerate(examples):
            ...
            if self.in_tokens:
                ...
                to_append = len(batch_records) < batch_size  # 32
            if to_append:
                batch_records.append(record)
            else:   # 每batch_size输出一次
    ...
    def data_generator(self,
        ...
        def wrapper():
            all_dev_batches = []
            for epoch_index in range(epoch):   # epoch=4
                if phase == "train":
                    self.current_example = 0
                    self.current_epoch = epoch_index  # 0,1,2,3
                if shuffle:
...
def npz_reader(f):
    data = {}
    for key, value in f.items():  # items() 函数以列表返回可遍历的(键, 值) 元组数组
        data[key] = value
    return data
...
class ClassifyReader(BaseReader):
    ...
    def _read_tsv(self, input_file, quotechar=None):
        ...
        with np.load(input_file) as f:
            ...
            examples = []
            for i in range(0, len(reader["sentence"])):
                line = [reader["sentence"][i], reader["aspect"][i], reader["label"][i]]
            ...
            if 'test_label' in input_file:
                input_path = os.path.dirname(input_file)
                name = ['Sentence', 'Aspect_Term', 'Sentiment_Polarity']
                test = pd.DataFrame(columns=name, data=examples)
                test.to_csv(os.path.join(input_path, 'test_true.csv'), encoding='utf-8')
