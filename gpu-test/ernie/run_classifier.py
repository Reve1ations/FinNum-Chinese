        ...
        while True:
            try:
                ...
                if nccl2_trainer_id == 0:
                    if steps % args.save_steps == 0:  # save_steps=10000
...
def predict_wrapper(args, reader, exe, test_prog, test_pyreader, graph_vars,
    ...
    for test_f, save_f in zip(test_sets, save_dirs):
        ...
        test_dir = os.path.dirname(args.test_set)
        example = pd.read_csv(os.path.join(test_dir, 'test_true.csv'), sep=',', encoding='utf-8')
        Example = namedtuple('Example', ['Sentence', 'Aspect_Term', 'Sentiment_Polarity'])
        pred_exam = []
        for s, a, p in zip(example["Sentence"], example["Aspect_Term"], preds):
            line = [s, a, p]
            for index, text in enumerate(line):
                line[index] = text
            examples = Example(*line)
            pred_exam.append(examples)

        name = ['Sentence', 'Aspect_Term', 'Sentiment_Polarity']
        test = pd.DataFrame(columns=name, data=pred_exam)  # 数据有3列，列名
        test.to_csv(save_path, encoding='utf-8')

        # path = os.path.join(os.path.dirname(test_dir), 'raw/test.xml')
        ...
        #             else:
        #                 preds[i] = "neutral"
        #             aspectTerm.set("polarity", preds[i])
        #             i = i + 1
        ...
        #             else:
        #                 preds[i] = "neutral"
        #             aspectCategory.set("polarity", preds[i])
        #             i = i + 1
